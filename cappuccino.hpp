...
struct {
  ...
  std::unordered_map<
      string, std::pair<std::function<Response(std::shared_ptr<Request>)>,
                        std::set<Method>>>
      routes;
...
std::vector<std::unique_ptr<std::string>> splitRequest(
    const std::string& str) noexcept {
  std::vector<std::unique_ptr<std::string>> result;
  std::string::size_type pos = str.find("\n", 0);
  if (pos != std::string::npos) {
    result.push_back(std::make_unique<std::string>(str.substr(0, pos)));
    ...
    if (sec_pos != std::string::npos) {
      ...
      result.push_back(
          std::make_unique<std::string>(str.substr(sec_pos, str.size())));
      return result;
    }
  }
  return result;
...
std::vector<std::unique_ptr<std::string>> split(const string& str,
...
std::pair<string, string> openFile(const string& aFilename) {
  ...
  } else if (response[0] == '\x89' && response[1] == 'P' &&
             response[2] == 'N' && response[3] == 'G') {
    return std::make_pair(response, "image/png");
  } else if (response[0] == 'G' && response[1] == 'I' && response[2] == 'F' &&
             response[3] == '8' && (response[4] == '7' || response[4] == '9')) {
...
class Request {
 ...
 public:
  Request(std::unique_ptr<string> aMethod, std::unique_ptr<string> aPath,
          std::unique_ptr<string> aProtocol, std::unique_ptr<string> abody)
      : method(move(aMethod)), protocol(move(aProtocol)), body(move(abody)) {
    ...
    if (path_param.size() == 2) {
      ...
      for (auto&& param : params) {
        ...
        if (param_key_val.size() == 2) {
          paramset.insert(make_pair(*param_key_val[0], *param_key_val[1]));
        }
      }
    }
  ...
  bool validateHttpVersion(const string& v) {
    ...
    if (v[0] != 'H' || v[1] != 'T' || v[2] != 'T' || v[3] != 'P') return false;
    if (v[4] != '/') return false;
  ...
  const string header(const string& key) {
    if (headerset.find(key) == headerset.end()) return "INVALID";
    return headerset[key];
  }

  const string params(const string& key) {
    if (paramset.find(key) == paramset.end()) return "INVALID";
    return paramset[key];
  }
  ...
  bool isCorrect() { return correctRequest; }
};
...
class Response {
 ...
 public:
  ...
  Response(int status, const string& msg, const string& protocol,
           const string& method, const string& body, const string& path)
      : status_(status),
...
string createResponse(string req) noexcept {
  ...
  if (lines.size() == 3) {
    request = std::shared_ptr<Request>(
        new Request(std::move(tops[0]), std::move(tops[1]), std::move(tops[2]),
                    std::move(lines[2])));
  } else {
    request = std::shared_ptr<Request>(
        new Request(std::move(tops[0]), std::move(tops[1]), std::move(tops[2]),
  ...
  if (!request->isCorrect()) {
    return Response(401, "Bad Request", *request->protocol, *request->method,
                    "<h1>Bad Request</h1>", *request->path);
  }

  if (context.routes.find(*request->path) != context.routes.end()) {
    auto f = context.routes[*request->path].first;
    if (context.routes[*request->path].second.find(method_of(
            *request->method)) != context.routes[*request->path].second.end() ||
        method_of(*request->method) == Method::HEAD) {
      return f(move(request));
    } else {
      return Response(405, "Method Not Allowed", *request->protocol,
                      *request->method, "<h1>Method Not Allowed!</h1>",
                      *request->path);
    }
  }
  return Response(404, "Not found", *request->protocol, *request->method,
                  "<h1>Not found!! </h1>", *request->path);
}
...
void load(const string& aDirectory, const string& filename) noexcept {
  ...
  if (dir != NULL) {
    ...
    context.routes.insert(std::make_pair(
        "/" + directory,
        std::make_pair<std::function<Response(std::shared_ptr<Request>)>,
                       std::set<Method>>(
            [directory, filename](
                std::shared_ptr<Request> request) -> Cappuccino::Response {
              auto file = openFile(directory);
              auto res = Response(200, "OK", "HTTP/1.1", "GET", file.first,
                                  "/" + directory);
...
void loadStaticFiles() noexcept { load(*context.static_root, ""); }

template <Method m>
void route(const string& path,
           std::function<Response(std::shared_ptr<Request>)> F) {
 ...
 public:
  AsioServer()
    ...
    asio::ip::tcp::endpoint endpoint =
        *resolver.resolve({context.address, std::to_string(context.port)});
        ...
        socket.async_read_some(
            ...
            [this, &buf](asio::error_code ec, std::size_t len) {
              if (!ec) {
                ...
                asio::async_write(socket, asio::buffer(createResponse(request)),
  ...
  void do_await_stop() {
    signals.async_wait([this](asio::error_code ec, int signo) {
      acceptor.close();
      socket.close();
    });
  }
